...
| G5 | 書き出し muxing が空 blob を返す | `export/export-engine.ts:529` | 🟡 | 音声は実装済 (WAV エンコーダ `exportAudioWAV`)。動画コンテナ muxer は次PR |
...
| G10 | EDL/FCPXML が export のみ・import 不在 (往復不可) | `interchange/legacy-formats.ts` | 🟡 | EDL importer 実装済 (往復OK)。FCPXML import は次PR |
